...
export const processWithdrawal = async (
    ...
    // Ép kiểu ID sang ObjectId để đồng bộ với DB
    ...
    if (status === "rejected") {
        ...
        await transactionService.createTransaction({
            userId: userObjectId, // Dùng ObjectId đã ép kiểu
        ...
        await transactionService.createTransaction({
            ...
            // Lưu ý: wallet.balance lúc này là số dư SAU KHI ĐÃ TRỪ (lúc tạo đơn)
            // Vậy balanceBefore (lúc chưa tạo đơn) phải là cộng ngược lại
...
export const getDashboardAnalytics = async ({ from, to } = {}) => {
    ...
    const [courses, enrollments, users, reviews] = await Promise.all([
        ...
        Enrollment.find(enrollmentQuery)
            .select(
                "courseId studentId progress completed createdAt completedAt",
            )
            .lean(),
        User.find(userQuery)
...
export const getDashboardCsv = async ({ from, to } = {}) => {
    ...
    const rows = [
        ...
        [
            "Summary",
            "Published Courses",
            dashboard.stats.totalPublishedCourses,
            "",
        ],
        [
            "Summary",
            "Archived Courses",
            dashboard.stats.totalArchivedCourses,
            "",
        ],
        ...
        [
            ...
            dashboard.stats.completedEnrollments,
            "",
        ],
        ...
        [
            ...
            dashboard.stats.averageCourseRating,
            "",
        ],
        ...
        [
            "Summary",
            "Total Certificates",
            dashboard.stats.totalCertificates,
            "",
        ],
        [""],
...
function sortLeaderboardItems(items = [], sortBy = "students") {
    ...
    return [...items].sort((a, b) => {
        ...
        if (
            ...
            return (
                Number(b.totalEnrollments || 0) -
                Number(a.totalEnrollments || 0)
            );
        }
        ...
        if (
            ...
            return (
                Number(b.estimatedRevenue || 0) -
                Number(a.estimatedRevenue || 0)
            );
        }
...
export const getInstructorLeaderboard = async ({
    ...
    enrollments.forEach((enrollment) => {
        ...
        uniqueStudentsByInstructor
            .get(instructorId)
            .add(String(enrollment.studentId));
    ...
    let items = [...leaderboardMap.values()].map((item) => {
        const uniqueStudents = uniqueStudentsByInstructor.get(
            item.instructorId,
        );
        const enrollmentsCount =
            enrollCounterByInstructor.get(item.instructorId) || 0;
        const completedCount =
            completionCounterByInstructor.get(item.instructorId) || 0;
