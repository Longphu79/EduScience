...
export default function useInstructorLessonManagePage() {
    ...
    const handleSubmit = useCallback(
        async (e) => {
            if (e) e.preventDefault(); // Kiểm tra e tồn tại để tránh lỗi nếu gọi thủ công
    ...
    // --- Cập nhật phần return để Page có thể sử dụng ---
    return {
        courseId,
        course,
        lessons,
        loading,
        saving,
        setSaving, // THÊM: Để Page bật loading khi upload video
        deletingId,
        editingLessonId,
        form,
        setForm, // THÊM: Để Page gán URL video sau khi upload thành công
        toast,
        setToast,
        stats,
        handleChange,
