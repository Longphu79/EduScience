...
export async function ensureConversation(courseId, studentId) {
    const response = await fetch(
        `${API_BASE_URL}/api/chat/course/${courseId}/conversation`,
        {
            method: "POST",
...
export async function getConversationMessages(conversationId) {
    const response = await fetch(
        `${API_BASE_URL}/api/chat/conversation/${conversationId}/messages`,
        {
...
export async function getInstructorCourseConversations(courseId) {
    const response = await fetch(
        `${API_BASE_URL}/api/chat/course/${courseId}/conversations`,
        {
            headers: createHeaders({}, true),
        },
    );
...
export async function listMyConversations() {
    const response = await fetch(`${API_BASE_URL}/api/chat/conversations/my`, {
        headers: createHeaders({}, true),
    });
...
export async function getMyUnreadSummary() {
    const response = await fetch(
        `${API_BASE_URL}/api/chat/conversations/unread-summary`,
        {
            headers: createHeaders({}, true),
        },
    );
...
export async function markConversationAsRead(conversationId) {
    const response = await fetch(
        `${API_BASE_URL}/api/chat/conversation/${conversationId}/read`,
        {
            method: "POST",
            headers: createHeaders(
                { "Content-Type": "application/json" },
