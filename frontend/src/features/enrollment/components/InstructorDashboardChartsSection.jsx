...
function TooltipBox({ active, payload, label }) {
  ...
  return (
    <div className="instructor-dashboard-chart-tooltip">
      {label ? <div className="instructor-dashboard-chart-tooltip__label">{label}</div> : null}
      {payload.map((item) => (
        <div key={item.dataKey} className="instructor-dashboard-chart-tooltip__row">
          <span>{item.name}: </span>
...
export default function InstructorDashboardChartsSection({
  ...
  const completionData = useMemo(
    () => [
      { name: "Completed", value: Number(summary.completedEnrollments || 0) },
      {
        name: "In Progress",
        value: Math.max(
          0,
          ...
          <div className={`${pageClassName}__chart-box`}>
            <ResponsiveContainer width="100%" height={340}>
              <BarChart data={topCoursesData}>
                ...
                <Tooltip content={<TooltipBox />} />
                <Legend />
                <Bar dataKey="avgQuizScore" name="Avg Quiz Score" fill="#10b981" radius={[8, 8, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        )}
          ...
          <div className={`${pageClassName}__chart-box`}>
            <ResponsiveContainer width="100%" height={360}>
              <BarChart data={trendData}>
                ...
                <Legend />
                <Bar dataKey="enrollments" name="Enrollments" fill="#6366f1" radius={[8, 8, 0, 0]} />
                <Bar dataKey="quizAttempts" name="Quiz Attempts" fill="#10b981" radius={[8, 8, 0, 0]} />
                <Bar dataKey="submissions" name="Submissions" fill="#f59e0b" radius={[8, 8, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        )}
