...
export default function useLearnCoursePage() {
    ...
    const lessons = useMemo(() => {
        const lessonList = Array.isArray(course?.lessonIds)
            ? course.lessonIds
            : [];
    ...
    const syncCurrentLesson = useCallback((courseData, enrollmentData) => {
        const lesson = getCurrentLessonFromEnrollment(
            courseData,
            enrollmentData,
        );
    ...
    const loadAssignmentsData = useCallback(async () => {
        ...
        try {
            ...
            setAssignmentSubmissions(
                normalizeSubmissionList(submissionResponse),
            );
        } catch (error) {
            setAssignments([]);
            setAssignmentSubmissions([]);
    ...
    const loadCertificate = useCallback(async () => {
        try {
            ...
            const response = await getCertificateByCourseStudent(
                courseId,
                studentId,
            );
    ...
    const handleCompleteLesson = useCallback(async () => {
        ...
        try {
            ...
            if (upcomingLessonId) {
                ...
                const currentLessonEnrollment = enrollmentUnwrap(
            ...
            showToast(
                error?.message || "Không thể hoàn thành bài học",
                "error",
            );
        } finally {
