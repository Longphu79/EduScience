    ...
    const loadData = useCallback(async () => {
        try {
            ...
            if (studentId && courseId) {
                try {
                    const attemptsRes =
                        await getAttemptsByStudentCourse(courseId);
                    const attemptList = normalizeAttemptList(
                        quizUnwrap(attemptsRes),
                    );

                    const quizAttempts = attemptList.filter(
                        (attempt) =>
                            String(getQuizId(attempt)) === String(quizId),
                    );
                    ...
                    if (forceRetake) {
                        ...
                        setQuizMode(
                            quizAttempts.length > 0 ? "result" : "taking",
                        );
                    }
                } catch (attemptError) {
            ...
            try {
                ...
                const payloadAnswers = (quiz.questions || []).map(
                    (question) => {
                        ...
                        const selectedOptionIndexes = Array.isArray(
                            answers[questionId],
                        )
                            ? answers[questionId]
                            : [];
                ...
                setAttempts((prev) =>
                    normalizeAttemptList([normalizedReview, ...prev]),
                );
